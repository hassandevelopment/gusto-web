...
const SPICE_OPTIONS = [
  { value: 0 as const, icon: '🚫', label: 'No spice', sub: 'Plain & mild' },
  { value: 1 as const, icon: '🌶', label: 'Mild', sub: 'Just a hint' },
  { value: 2 as const, icon: '🌶🌶', label: 'Medium', sub: 'Building heat' },
  { value: 3 as const, icon: '🌶🌶🌶', label: 'Hot', sub: 'Fire it up' },
]
...
export default function ItemCustomizeSheet({ item, onClose }: Props) {
  ...
  if (!item) {
    return (
      <dialog
        ref={dialogRef}
        style={{ position: 'fixed', inset: 'auto 0 0 0', width: '100%', maxWidth: '100%', maxHeight: '92dvh', margin: 0, padding: 0, border: 'none', borderRadius: '20px 20px 0 0', overflow: 'hidden' }}
  ...
  function handleAddToOrder() {
    if (!item) return
    const notes = buildNotes([...removed], [...added], spice)
    add(item.id, qty, notes || undefined)
    onClose()
  }
        ...
        <>
          ...
          <div className="absolute top-4 right-4 z-10">
            <button
              ...
              onClick={onClose}
              aria-label="Close"
              className="w-9 h-9 rounded-full bg-ink/60 backdrop-blur flex items-center justify-center
          ...
          <div className="overflow-y-auto overscroll-contain flex-1 px-5 pt-4 pb-2">
            ...
            {item.tags && item.tags.length > 0 && (
              <div className="flex flex-wrap gap-1.5 mb-3">
                {item.tags.map((tag) => (
                  <span
                    key={tag}
                    className="text-[11px] font-semibold uppercase tracking-[0.07em] px-2 py-0.5 rounded-full bg-bg text-text-muted border border-[rgba(104,90,90,0.18)]"
                  >
                    {TAG_LABELS[tag] ?? tag}
                  </span>
                ))}
            ...
            {/* Description */}
            {item.description && (
              <p className="text-text text-sm leading-relaxed mb-4" style={{ fontFamily: 'var(--font-sans)' }}>
                {item.description}
              </p>
            )}
            ...
            {/* Price */}
            <p
              className="text-ink tabular-nums mb-1"
                      ...
                      <button
                        ...
                        className={`px-3 py-1.5 rounded-full text-sm font-medium border transition-all duration-150 cursor-pointer select-none ${
                          active
                            ...
                            : 'bg-bg text-text-muted border-[rgba(104,90,90,0.18)] line-through'
                        }`}
                      >
                        {ing}
                      </button>
                ...
                return (
                  <button
                    ...
                    className={`rounded-2xl p-4 text-left border-2 transition-all duration-200 cursor-pointer select-none ${
                      active
                        ? 'bg-ink text-white border-ink'
                        : 'bg-bg text-ink border-[rgba(104,90,90,0.15)] hover:border-[rgba(104,90,90,0.35)]'
                    }`}
                  >
                    ...
                    <p
                      className={`text-xs ${active ? 'text-white/60' : 'text-text-muted'}`}
                    >
                      {opt.sub}
                    </p>
                  </button>
